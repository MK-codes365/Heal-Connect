                    ...
                    <div key={doctor.id} className="doctor-card">
                        ...
                        <div className="doctor-info">
                            ...
                            <div className="doctor-stats">
                                <span><FaStar style={{ color: '#f59e0b' }} /> {doctor.rating}</span>
                                <span>{doctor.consultations}+ consultations</span>
                            </div>
                            <p className="fees">₹{doctor.fees} per consultation</p>
                            <p className="languages">Languages: {doctor.languages.join(', ')}</p>
                        </div>
                        <div className="doctor-actions">
                            <button 
                                className={`consult-btn ${doctor.availability === 'Available' ? 'available' : 'busy'}`}
                                onClick={() => navigate('/dashboard/patient/book', { state: { doctor } })}
                                disabled={doctor.availability !== 'Available'}
                            >
                                <FaVideo /> {doctor.availability === 'Available' ? 'Consult Now' : 'Busy'}
                            </button>
                            <button 
